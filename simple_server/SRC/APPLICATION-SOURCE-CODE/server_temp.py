...
def search_return_html():
    ...
    return render_template('searchResults_template.html', count=rows_num, query=query)
@app.route('/search2')
def search_return_html2():
    query = request.args.get('query')
    cur = mysql.cursor()
    mysql_query = f"SELECT * FROM Film WHERE Title LIKE '%{query}%' LIMIT 50"
    cur.execute(mysql_query)
